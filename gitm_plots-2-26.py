...
def make_a_keo(arr, title, cbarlims, cbar_name, 
    ...
    """
    if fname != None and os.path.exists(fname) and save_or_show == 'save':
        if not OVERWRITE:
...
def draw_map(data_arr, title, cbarlims, cbar_label = None,
             y_label = 'Latitude (deg)', x_label = 'Longitude (deg)', save_or_show = 'save' , fname = None,
...
def thread_call_maps(args):
    call_maps(alt_idx=args[0], namecol = args[1], dtime_real = args[2], outliers = args[3], diffs = args[4], save_or_show = 'save', return_figs = False, figtype = 'all')
        
def loop_maps(sel_cols, sel_alts, save_or_show = 'show', return_figs = False, figtype = 'all', thread = False, outliers = False, diff = None):
    #make sure inputs are lists so we can loop through them.
    if type(sel_cols) is not list: 
        if type(sel_cols) is not np.array:
            sel_cols = [ sel_cols ]
    ...
    if not thread:  # make plots sequentially
        ...
        for namecol in sel_cols:
            ...
            for nalt in sel_alts:
                for dtime_real in times:
                    call_maps(nalt, dtime_real = dtime_real, numcol = numcol, save_or_show = save_or_show, return_figs = return_figs, figtype = figtype, outliers = outliers, diffs = diff)
                    pbar.update(1)
        ...
        for namecol in sel_cols:
            ...
            for nalt in sel_alts:
                for dtime_real in times:
                    arg_arr.append([nalt,namecol,dtime_real,outliers, diff])
...
# %
# [0,0,:,:,5]
# call_maps(28, dtime_index = 50, numcol = 0, save_or_show='show', figtype='diff', diffs=diff_vs)
...
loop_maps(gitm_cols, gitm_alt_idxs, save_or_show='save', thread = True, diff = diff_vs)
...
# %
loop_maps(gitm_cols, gitm_alt_idxs, save_or_show='save', thread = True, outliers = True, diff = diff_vs)
